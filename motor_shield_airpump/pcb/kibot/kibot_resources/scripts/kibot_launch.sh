...
output_dir="."
kibot_base="kibot"
kibot_config="-c 'pcb/kibot/kibot_yaml/kibot_main.yaml'"
schematic_file="-e pcb/$KB_PROJECT_FILE.kicad_sch"
revision=""
costs_flag=false
server_flag=false
server_port=8000
...
if [[ -z "$revision" ]]; then
    revision=$(python3 pcb/kibot/kibot_resources/scripts/get_changelog_version.py -f pcb/Changelog.md)
    if [[ $? -ne 0 ]]; then
...
if [[ "$costs_flag" == true ]]; then
    ...
    case "$variant" in
        DRAFT)
            kibot_command1="$kibot_base $schematic_file --skip-pre set_text_variables,draw_fancy_stackup,erc,drc $kibot_config -d '$output_dir' -g variant=$variant -E REVISION='$revision' md_readme"
            kibot_command2="$kibot_base $schematic_file --skip-pre draw_fancy_stackup,erc,drc $kibot_config -d '$output_dir' -g variant=$variant -E REVISION='$revision' draft_group"
            ;;
        PRELIMINARY)
            kibot_command1="$kibot_base $schematic_file --skip-pre erc,drc $kibot_config -d '$output_dir' -g variant=$variant -E REVISION='$revision' notes"
            kibot_command2="$kibot_base $schematic_file --skip-pre erc,drc $kibot_config -d '$output_dir' -g variant=$variant -E REVISION='$revision' all_group"
            ;;
        CHECKED|RELEASED|*)
            kibot_command1="$kibot_base $schematic_file --skip-pre set_text_variables,draw_fancy_stackup,erc,drc $kibot_config -d '$output_dir' -g variant=$variant -E REVISION='$revision' notes"
            kibot_command2="$kibot_base $schematic_file $kibot_config -d '$output_dir' -g variant=$variant -E REVISION='$revision' $all_group"
            ;;
    esac
fi
